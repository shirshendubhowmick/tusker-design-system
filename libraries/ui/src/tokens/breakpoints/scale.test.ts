...
describe('breakpoints', () => {
  it('defines mobile / tablet / desktop in order', () => {
    ...
    expect(Object.keys(breakpoints).sort()).toEqual([...breakpointOrder].sort());
  });

  it('uses contiguous mobile-first ranges', () => {
    expect(breakpoints.mobile.minWidthPx).toBe(0);
    expect(breakpoints.mobile.maxWidthPx).toBe(breakpoints.tablet.minWidthPx - 1);
    expect(breakpoints.tablet.maxWidthPx).toBe(breakpoints.desktop.minWidthPx - 1);
  ...
  it('Storybook viewports align with product tiers', () => {
    ...
    for (const name of breakpointOrder) {
      ...
      const width = Number.parseInt(designSystemViewports[name].styles.width, 10);
